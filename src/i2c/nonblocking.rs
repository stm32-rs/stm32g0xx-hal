...
use super::{EndMarker, Error, I2c, I2cDirection, I2cExt, I2cResult, Instance, SCLPin, SDAPin};
use crate::gpio::*;
        ...
        SCL: SCLPin<I2C>,
    {
        ...
        if config.slave_address_1 > 0 {
            i2c.oar1().write(|w| unsafe {
                w.oa1()
                    .bits(config.slave_address_1)
                    .oa1mode()
                    .bit(config.address_11bits)
                    .oa1en()
                    .set_bit()
            });
        ...
        if config.slave_address_2 > 0 {
            i2c.oar2().write(|w| unsafe {
                w.oa2msk()
                    .bits(config.slave_address_mask as u8)
                    .oa2()
                    .bits(config.slave_address_2)
                    .oa2en()
                    .set_bit()
            });
            // Enable acknowlidge control
...
impl<I2C: Instance, SDA, SCL> I2cControl for I2c<I2C, SDA, SCL> {
...
impl<I2C: Instance, SDA, SCL> I2cMaster for I2c<I2C, SDA, SCL> {
    fn master_write(&mut self, addr: u16, data: &[u8]) -> nb::Result<(), Error> {
...
impl<I2C: Instance, SDA, SCL> I2cSlave for I2c<I2C, SDA, SCL> {
    ...
    fn set_address(&mut self, address: u16) {
        self.i2c
            .oar1()
            .write(|w| unsafe { w.oa1().bits(address).oa1en().clear_bit() });
