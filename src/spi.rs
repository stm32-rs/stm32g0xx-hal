...
impl<SPI: Instance> SpiExt for SPI {
    fn spi<PINS>(self, pins: PINS, mode: Mode, freq: Hertz, rcc: &mut Rcc) -> Spi<SPI, PINS>
    where
        PINS: Pins<SPI>,
    {
...
impl<SPI: Instance, PINS> ErrorType for Spi<SPI, PINS> {
    type Error = Error;
}

impl<SPI: Instance, PINS> SpiBus for Spi<SPI, PINS> {
